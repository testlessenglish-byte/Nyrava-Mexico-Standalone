...
export const createCheckoutSession = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: unknown) =>
    z
      .object({
        ...
        origin: z.string().url(),
        // Total attorney seats requested. Only used by plans that expose a
        // per-seat add-on price; otherwise ignored and the base price ships alone.
  ...
  .handler(async ({ data, context }) => {
    ...
    // Look up the admin-managed plan row for this key, including seat metering.
    const { data: planRow, error: planErr } = await admin
      .from("billing_plans")
      .select(
        "key,label,stripe_price_id,self_serve,active,included_seats,per_seat_price_cents,per_seat_stripe_price_id",
      )
      .eq("key", data.planKey)
      .maybeSingle();
    ...
    // Seat metering. Row may be missing columns on older projects; coerce
    // defensively so plans without add-ons keep the current single-line shape.
    const includedSeats =
      typeof (planRow as { included_seats?: number | null }).included_seats === "number"
        ? Math.max(1, (planRow as { included_seats: number }).included_seats)
        : 1;
    const perSeatPriceId =
      (planRow as { per_seat_stripe_price_id?: string | null }).per_seat_stripe_price_id ?? null;
    const seats = Math.max(includedSeats, data.seats ?? includedSeats);
    ...
    const lineItems: LineItem[] = [{ price: priceId, quantity: 1 }];
    if (extraSeats > 0 && perSeatPriceId) {
      lineItems.push({ price: perSeatPriceId, quantity: extraSeats });
    }
