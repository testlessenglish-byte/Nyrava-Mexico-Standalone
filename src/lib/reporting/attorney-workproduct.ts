...
export type FindingSourceDoc = { name: string; weight: EvidenceWeight; quotes: string[] };
...
const WEIGHT_RULES: Array<{ re: RegExp; w: EvidenceWeight }> = [
  { re: /(sentencia|resolucion|laudo|auto de|acuerdo judicial|interlocutoria|ejecutoria)/, w: JUDICIAL_RESOLUTION_WEIGHT },
...
const MATERIA_LABELS: Record<string, string> = {
  penal: "penal", penal_adolescentes: "penal para adolescentes", civil: "civil", general_civil: "civil",
  mercantil: "mercantil", mercantil_concursal: "concursal", mercantil_corporativo: "corporativa",
  laboral: "laboral", familiar: "familiar", familiar_sucesiones: "sucesoria", amparo: "de amparo",
  constitucional: "constitucional", fiscal: "fiscal", administrativo: "administrativa", agrario: "agraria",
  ambiental: "ambiental", migratorio: "migratoria", electoral: "electoral", consumidor: "de protección al consumidor",
  inmobiliario: "inmobiliaria", propiedad_intelectual: "de propiedad intelectual", maritimo: "marítima",
};
export function materiaLabel(caseType?: string | null): string {
  return MATERIA_LABELS[norm(caseType ?? "").replace(/[^a-z_]/g, "")] ?? "";
}
...
/**
 * Resolve the source documents already proven by finding provenance. Some
 * engine rows carry only doc_n/source_document_id + quote, while the export
 * inventory carries the filename separately. When the corpus contains exactly
 * one document, a non-empty source_document_id is enough to map that finding
 * to the sole inventory label without guessing. This closes the real ADR
 * failure where a verified SCJN holding was shown as having "sin fuente
 * documental" merely because evidence_refs omitted a redundant filename.
 */
function findingSourceDocs(f: AnyFinding, ctx?: WorkProductContext): Array<{ name: string; quotes: string[] }> {
  const refs = Array.isArray(f.evidence_refs) ? f.evidence_refs : [];
  const byName = new Map<string, string[]>();
  const unlabeledQuotes: string[] = [];
  for (const r of refs) {
    const quote = String(r?.quote ?? r?.excerpt ?? r?.snippet ?? r?.text ?? "").trim();
    const label = String(r?.filename ?? r?.label ?? r?.document_title ?? "").trim();
    if (!label) {
      if (quote && !unlabeledQuotes.includes(quote)) unlabeledQuotes.push(quote);
      continue;
    }
  ...
  if (byName.size === 0 && ctx?.documentLabels.length === 1 && String(f.source_document_id ?? "").trim()) {
    const name = ctx.documentLabels[0];
    const sourceQuote = String(f.source_quote ?? "").trim();
    const quotes = [...unlabeledQuotes];
    if (sourceQuote && !quotes.includes(sourceQuote)) quotes.push(sourceQuote);
    byName.set(name, quotes);
  }
  return [...byName.entries()].map(([name, quotes]) => ({ name, quotes }));
}

function findingText(f: AnyFinding): string {
  return norm([f.title, f.description, f.legal_significance, f.potential_impact, f.category].filter(Boolean).join(" \n "));
}
...
export function groupForFinding(f: AnyFinding): AttorneyGroupKey {
  ...
  if (FAVORABLE_RE.test(text) && Number(f.confidence ?? 0) >= 0.7) return "favorable";
  return "revision";
}
...
function buildActions(f: AnyFinding, docs: FindingSourceDoc[], pending: string[]): string[] {
  ...
  if (top) {
    ...
    if (top.weight.stars < 4) actions.push(`Obtener copia certificada de ${top.name} para elevar su valor probatorio.`);
    if (top.weight.stars <= 2) actions.push("Confirmar la autenticidad del soporte documental (ratificación, certificación notarial o cotejo con el original).");
  } else {
    actions.push("Identificar y adjuntar al expediente el documento fuente que sustenta este hallazgo.");
  }
  if (CONTRADICTION_RE.test(cat) || CONTRADICTION_RE.test(text)) actions.push("Comparar de manera directa los documentos contradictorios y documentar cuál tiene mayor valor probatorio.");
  if (/(cadena|custodia|traslado|resguardo)/.test(text)) actions.push("Revisar la cadena documental completa, desde el origen del documento hasta su incorporación al expediente.");
  if (/(cfdi|factura|contabilidad|nomina|pago)/.test(text)) actions.push("Revisar los CFDI y registros contables relacionados con las cantidades referidas.");
  if (/(notificacion|emplazamiento|acuse)/.test(text)) actions.push("Confirmar los acuses de notificación y su constancia en autos.");
  if (/(plazo|termino|fecha|caducidad|prescripcion)/.test(text)) actions.push("Confirmar las fechas procesales aplicables contra las constancias del expediente.");
  if (/(autoridad|administrativ|sat|imss|infonavit|impi|inm|profeco|comar)/.test(text)) actions.push("Solicitar el expediente administrativo completo a la autoridad correspondiente.");
  if (pending.length) actions.push("Solicitar o localizar la documentación pendiente identificada para este hallazgo.");
  if (docs.length <= 1) actions.push("Localizar evidencia adicional e independiente que corrobore el hallazgo dentro del expediente.");
  const unique = Array.from(new Set(actions));
  if (unique.length < 3) unique.push("Cotejar el hallazgo con el resto de las constancias del expediente antes de utilizarlo.");
  if (unique.length < 3) unique.push("Someter el hallazgo a revisión del abogado responsable del asunto.");
  return unique.slice(0, 7);
}

function buildImportance(f: AnyFinding, docs: FindingSourceDoc[], ctx: WorkProductContext, detail: EvidenceSynthesis | null): string[] {
  const paras: string[] = [];
  ...
  paras.push(
    `El hallazgo se clasifica con gravedad ${sev}` +
      ...
      (f.legal_significance ? String(f.legal_significance) : "Su relevancia deriva de que el punto descrito está sustentado en constancias que ya obran en el expediente."),
  );
  const strongest = docs[0];
  const dir = norm(f.impact_direction ?? "");
  const effect = dir === "strengthens" ? "refuerza la posición probatoria de la parte a la que beneficia" : dir === "weakens" ? "debilita el sustento probatorio del punto controvertido" : "modifica el peso probatorio que puede atribuirse al punto controvertido";
  paras.push(
    docs.length > 0
      ? `Con base en las fuentes que lo sustentan (${docs.length} documento(s); el de mayor peso probatorio es ${strongest.name} — ${strongest.weight.label} ${strongest.weight.glyphs}), el hallazgo ${effect}. ` +
        (docs.length === 1 ? "Al descansar en una sola fuente, su fuerza probatoria depende de que se localice corroboración independiente." : detail?.conflicts.length ? `Los documentos citados divergen en ${detail.conflicts.length} dato(s) concreto(s), por lo que la discrepancia debe resolverse antes de apoyarse en el hallazgo.` : detail?.agreements.some((a) => a.independent) ? "Los datos extraídos coinciden entre documentos de distinta naturaleza probatoria, lo que constituye corroboración independiente del punto." : detail?.agreements.length ? "Los datos extraídos coinciden entre documentos de la misma naturaleza probatoria, lo que constituye soporte acumulativo y no corroboración independiente." : "Los pasajes citados no contienen datos concretos comparables entre sí, por lo que no puede afirmarse que las fuentes se corroboren entre ellas.")
      : `El hallazgo ${effect}; sin embargo, no se identificó en el expediente una fuente documental citada de manera directa, por lo que su valor probatorio no puede considerarse acreditado en este estado del expediente.`,
  );
  if (PROCEDURAL_RE.test(text)) paras.push("El hallazgo plantea además una cuestión de orden procesal: incide en formalidades, plazos o constancias del procedimiento cuya verificación en autos debe realizarse antes de cualquier actuación que dependa de ellas.");
  else if (f.potential_impact) paras.push(String(f.potential_impact));
  const needsReview = conf < 0.75 || docs.length <= 1 || String(f.verification_status ?? "") === "disputed";
  paras.push(needsReview ? "Se requiere revisión profesional antes de apoyarse en este hallazgo: el sustento documental es limitado o su confiabilidad no alcanza el nivel más alto que el sistema puede confirmar contra el corpus." : "El hallazgo cuenta con sustento documental múltiple y confiabilidad alta dentro del corpus analizado; corresponde al abogado confirmar su vigencia procesal antes de utilizarlo.");
  return paras.slice(0, 4);
}

function buildSynthesis(f: AnyFinding, docs: FindingSourceDoc[], ctx: WorkProductContext): { synthesis: FindingWorkProduct["synthesis"]; detail: EvidenceSynthesis | null } {
  ...
  return { synthesis: { docs, narrative: detail.narrative, lines: detail.lines, grounded: detail.grounded }, detail };
}
...
export function buildFindingWorkProduct(f: AnyFinding, ctx: WorkProductContext): FindingWorkProduct {
  ...
  return { importance: buildImportance(f, docs, ctx, detail), synthesis, synthesisDetail: detail, pending, actions: buildActions(f, docs, pending), group: groupForFinding(f) };
}
...
export function buildCaseSnapshot(findings: AnyFinding[], ctx: WorkProductContext): CaseSnapshot {
  ...
  const strengths = enriched.filter(({ wp }) => !wp.synthesisDetail?.conflicts.length && (wp.synthesisDetail?.agreements.some((a) => a.independent) ?? false)).slice(0, 5).map(({ f, wp }) => {
  ...
  const weaknesses = enriched.filter(({ f, wp }) => (wp.synthesis?.docs.length ?? 0) <= 1 || Number(f.confidence ?? 0) < 0.6 || (wp.synthesisDetail?.conflicts.length ?? 0) > 0 || !(wp.synthesisDetail?.agreements.length ?? 0)).slice(0, 5).map(({ f, wp }) => {
    const d = wp.synthesisDetail;
    if (d?.conflicts.length) return `${title(f)} — los documentos citados divergen en ${d.conflicts.length} dato(s) concreto(s).`;
    if (!wp.synthesis?.docs.length) return `${title(f)} — sin fuente documental citada de manera directa.`;
    if (wp.synthesis.docs.length === 1) return `${title(f)} — fuente documental única (${wp.synthesis.docs[0].weight.label}).`;
...
export function buildExecutiveQuestions(findings: AnyFinding[], ctx: WorkProductContext, snapshot: CaseSnapshot): SnapshotQuestion[] {
  const materia = materiaLabel(ctx.caseType);
