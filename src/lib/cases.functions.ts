  ...
  .inputValidator((d: unknown) =>
    z
      .object({
        ...
        perspective: z
          .enum(["defense", "prosecution", "plaintiff", "respondent", "appellate", "jury", "independent"])
          .optional(),
      })
      .parse(d),
  )
...
export const askCaseAi = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: unknown) =>
    z.object({ caseId: z.string().uuid(), question: z.string().min(1).max(4000) }).parse(d),
  )
  .handler(async ({ data, context }) => {
...
export const getAiHealth = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const env = assertEnv();
    const active =
      keys.length > 0 ? "groq only (user keys)" : diag.providers.groq.configured ? "groq only (env)" : null;
    return {
      providers: {
        groq: {
          ...groq,
  ...
  .handler(async ({ data, context }) => {
    ...
    if (currentReport) {
      ...
      const [{ count: docCount }, { count: findCount }] = await Promise.all([
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (supabase as any).from("case_findings").select("id", { count: "exact", head: true }).eq("case_id", caseId),
      ]);
