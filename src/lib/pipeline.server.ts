...
${corpus.slice(0, s(160000))}`;
  ...
  const systemInstruction =
    ...
    '\nGOOD: {"issue":"Whether Officer Reyes\' warrantless search of Ellison\'s vehicle violated the Fourth Amendment under Arizona v. Gant","rule":"Under Arizona v. Gant, 556 U.S. 332 (2009), police may search a vehicle incident to arrest only if the arrestee is within reaching distance at the time of the search, or if it is reasonable to believe evidence relevant to the crime of arrest might be found in the vehicle","application":"Here, Officer Reyes searched at 15:08, approximately 45 minutes after Ellison was handcuffed and secured in the patrol vehicle, per the report\'s note that the \'subject was secured in patrol vehicle at 14:23\' [DOC 2 p.4]. Ellison was not within reaching distance. The crime of arrest was speeding, making it unreasonable to believe the vehicle contained evidence of that offense.","conclusion":"The search violated Gant. A motion to suppress the firearm recovered from the glove compartment should be filed.","cited_evidence":["DOC 2 p.4","DOC 5 p.1"]}' +
    '\nBAD (do NOT produce): {"issue":"Fourth Amendment issue","rule":"The Fourth Amendment protects against unreasonable searches","application":"The search was unreasonable because the defendant was already arrested","conclusion":"Motion to suppress should be filed"}' +
...
${paginationTail}`;
  ...
  if (proseLooksEmpty) {
    ...
    const byCategory = (cats: string[]) =>
      [...findings]
        .filter((f) => cats.includes(String((f as any).category ?? "")))
